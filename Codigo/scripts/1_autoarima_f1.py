...
EXPERIMENT_CONFIG = {
    "tracking_uri": "",
    "experiment_name": "TFM_Modelado_ARIMA_F1",
    "test_size": 6,
    "future_steps": 6,
    "seasonal_periods": [6, 24, 144, 1008],
    "predictions_output_path": "../predicciones_autoarima_f1.csv",
}
...
ts_1 = ts_1.interpolate(method="time").ffill().bfill()

print("Observaciones finales:", len(ts_1))
print(ts_1.tail(5).to_frame(name=TARGET_COL))
...
with TemporaryDirectory(prefix="mlflow_autoarima_") as temp_artifacts_dir:
    ...
    with mlflow.start_run(run_name="autoarima_fase1_parent"):
        ...
        ax.plot(
            np.asarray(ts_1.index),
            np.asarray(ts_1.values, dtype=float),
            color="tab:blue",
            linewidth=1,
        )
        ...
        series_stats = {
            "mean": float(ts_1.mean()),
            "std": float(ts_1.std()),
            "min": float(ts_1.min()),
            "max": float(ts_1.max()),
            "median": float(ts_1.median()),
            "q25": float(ts_1.quantile(0.25)),
            "q75": float(ts_1.quantile(0.75)),
            "skew": float(ts_1.skew()),
            "kurtosis": float(ts_1.kurtosis()),
        }
        ...
        print("\n" + "=" * 60)
        print("RESUMEN FINAL DEL EXPERIMENTO - FASE 1")
        print("=" * 60)
        ...
        print(f"\nTotal de observaciones: {len(ts_1)}")
